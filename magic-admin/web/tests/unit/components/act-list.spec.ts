...
//   // 等待获取活动列表
//   await flushPromises();
//   const nextPageBtn = wrapper.find('button[class="btn-next"]');
//   // 模拟点击
...
// it('按页面标题查询', async () => {
//   const wrapper = getWrapper();
//   const pageTitleInput = wrapper.find('input[placeholder="页面标题"]');

//   await pageTitleInput.setValue('title');
//   // 模拟回车
//   await pageTitleInput.trigger('keydown.enter');
//   await flushPromises();

//   expect((actApi.getList as jest.Mock).mock.calls[1][0].data.where.pageTitle).toBe('title');
// });

// it('创建活动', async () => {
//   const wrapper = getWrapper();
//   const dialog = wrapper.findComponent(FormDialog);
//   dialog.vm.$emit('afterAction', { data: { actId: 1 } });
//   await flushPromises();
//   expect((actApi.getList as jest.Mock).mock.calls.length).toBe(2);
//   expect(vueRouter.useRoute().path).toBe('/editor/1');
// });
...
// it('排序', async () => {
//   const wrapper = getWrapper();
//   const table = wrapper.findComponent(MTable);
//   table.vm.$emit('sort-change', { prop: 'test', order: 'asc' });
//   await flushPromises();
//   expect((actApi.getList as jest.Mock).mock.calls[1][0].data.orderBy[0]).toEqual({
//     columnName: 'test',
//     direction: 'asc',
//   });
// });
...
// it('无权限复制', () => {
//   Cookies.set('userName', 'otherUser');
//   const wrapper = getWrapper();
...
// it('修改页面容量', async (done) => {
//   const wrapper = getWrapper();
//   const select = wrapper.findAllComponents(ElSelect)[1];
//   await select.vm.$emit('change', 20);
//   setTimeout(() => {
//     const query = (actApi.getList as jest.Mock).mock.calls[1][0].data;
//     console.log('---query----', query);
//     expect(query.pgSize).toBe(20);
//     done();
//   }, 100);
...
// const newActBtn = wrapper.find('button');
// // 模拟点击
// await newActBtn.trigger('click');

// expect(dialog.attributes('model-value')).toBe('true');
// });
...
//   const copyBtn = buttons.find((btn) => btn.text() === '复制');
//   copyBtn?.trigger('click');
//   expect(actApi.copyAct).toBeCalled();
// });
...
// it('复制失败-res不存在', () => {
//   (actApi as any).setCopyRet(undefined);
//   const wrapper = getWrapper();
//   const buttons = wrapper.findAll('button');
//   const copyBtn = buttons.find((btn) => btn.text() === '复制');
//   copyBtn?.trigger('click');
